...
class GameServer:
    ...
    def accept_connections(self):
        ...
        try:
            while True:
                ...
                with self.client_lock, self.game_state_lock:
                    if len(self.game_state["players"]) >= MAX_PLAYERS:
                        self.send_message_to_client(client_socket, {"type": "connection_rejected", "message": "Game is full"})
                        client_socket.close()
                        continue
                    ...
                    new_player = {"id": player_id, "x": position["x"], "y": position["y"], "speed": BASE_SPEED, "score": 0, "color": position["color"], "hasObject": False, "powerups": {"speedBoost": 0, "speedPenalty": 0}}
                    self.game_state["players"].append(new_player)
                    self.clients[client_id] = (client_socket, address, player_id)
                    self.send_message_to_client(client_socket, {"type": "connection_accepted", "playerId": player_id, "gameState": self.game_state})
    ...
    def generate_obstacles(self):
        ...
        while len(obstacles) < NUM_OBSTACLES:
            new_obstacle = {"x": random.uniform(0, CANVAS_SIZE - PLAYER_SIZE * 4), "y": random.uniform(0, CANVAS_SIZE - PLAYER_SIZE * 4), "size": PLAYER_SIZE * 2, "type": "ice"}
            overlapping = False
        ...
        for _ in range(NUM_POWERUPS // 2):
            while True:
                ...
                if not any(math.hypot(p["x"] - x, p["y"] - y) < 30 * 5 for p in powerups) and \
                   not any(self.check_collision(x, y, POWERUP_SIZE, obs["x"], obs["y"], obs["size"]) for obs in obstacles) and \
    ...
    def broadcast_game_state(self):
        # Step 1: Copy state under game_state_lock
        with self.game_state_lock:
            game_state_copy = json.loads(json.dumps(self.game_state))  # deep copy to prevent mutation

        # Step 2: Send to all clients under client_lock
        with self.client_lock:
